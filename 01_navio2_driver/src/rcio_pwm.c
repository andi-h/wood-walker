...
#else
    static int rcio_pwm_apply(struct pwm_chip *chip, struct pwm_device *pwm_dev, const struct pwm_state *state);
    static int rcio_pwm_get_state(struct pwm_chip *chip, struct pwm_device *pwm_dev, struct pwm_state *state);
#endif

static ssize_t heartbeat_store(struct device *pwm_dev, struct device_attribute *attr, const char *buf, size_t size);
...
static ssize_t heartbeat_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    long unsigned int heartbeat_value;
    
    int ret = kstrtoul(buf, 10, &heartbeat_value);
    if (ret < 0)
        return ret;

    if(heartbeat_value == 1) 
    { 
...
int rcio_pwm_probe(struct rcio_state *state)
{
    ...
    if (!pwm->chip.dev) {
        pr_err("PWM chip device is NULL\n");
        pwmchip_remove(&pwm->chip); 
        return -EINVAL;
    }

    // add custom heartbeat attribute since the config method from old kernel versions is no longer available and the new apply method is just called when settings are changed.
    ret = sysfs_create_file(&pwm->chip.dev->kobj, &dev_attr_heartbeat.attr);
    if (ret)
    {
        pwmchip_remove(&pwm->chip);
...
{
    sysfs_remove_file(&pwm->chip.dev->kobj, &dev_attr_heartbeat.attr);
    
    #if (LINUX_VERSION_CODE < KERNEL_VERSION(4,19,0))
        int ret = pwmchip_remove(&pwm->chip);
...
#define inside_range(x, lower, upper) ((x >= lower) && (x <= upper))
    ...
    static int rcio_pwm_apply(struct pwm_chip *chip, struct pwm_device *pwm_dev,
                            const struct pwm_state *state)
    {
        if (state->enabled) {
            rcio_pwm_config(chip, pwm_dev, state->duty_cycle, state->period);
            armed = true;
        } 
        else {
            // Disable PWM
    ...
    static int rcio_pwm_get_state(struct pwm_chip *chip, struct pwm_device *pwm_dev,
                                struct pwm_state *state)
    {
        int pwm_group_number = 0;

        if (inside_range(pwm_dev->hwpwm, 0, 3)) pwm_group_number = 0;
        if (inside_range(pwm_dev->hwpwm, 4, 7)) pwm_group_number = 1;
        if (inside_range(pwm_dev->hwpwm, 8, 11)) pwm_group_number = 2;
        if (inside_range(pwm_dev->hwpwm, 12, 15)) pwm_group_number = 3;

        state->period = 1000000000 / frequencies[pwm_group_number];
